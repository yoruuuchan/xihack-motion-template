...
**DIRECTION LOCKED，DETAILS NOT APPROVED（测试合成 `DialTest` / `src/scenes/Dial.tsx`）**：旋钮像滚筒洗衣机的档位选择器；扭到一个档位即选择对应主题，并触发缩放、推进或其他转场进入该主题场景。旋钮只在“选择 / 跳转”的必要时刻出现，进入场景后应离场，不能常驻整片。当前测试只覆盖“待机 → 团队 → 问题”，旋钮尺寸、材质、排版、运镜、按压反馈与离场方式均尚未逐项批准。

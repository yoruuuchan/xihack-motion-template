  ...
  return (
    <Shell bg={C.blue}>
      ...
      <div style={{position: 'absolute', left: 80, top: 52, color: onBase ? C.ink : 'white', opacity: arrive}}>
        <Small>{data.event} <span style={{opacity: 0.5}}> / </span> {data.location}</Small>
      </div>
      <div
        style={{
          position: 'absolute',
      ...
      >
        <div style={{display: 'flex', alignItems: 'center', gap: 40}}>
          ...
          <div style={{fontSize: 174, fontWeight: 700, letterSpacing: -6}}>{data.team}</div>
        </div>
      ...
      <div style={{position: 'absolute', left: 116, top: 318, opacity: reveal, transform: `translateY(${105 * (1 - reveal) - exit * 25}px)`}}>
        ...
        <div style={{fontSize: 260, lineHeight: 1.12, fontWeight: 700, letterSpacing: -16, marginLeft: -11, marginTop: 14}}>
          {data.project}<span style={{color: C.teal}}>.</span>
        </div>
        <div style={{fontSize: 47, marginTop: 20, opacity: p(f, 131, 155)}}>{data.tagline}</div>
      </div>
